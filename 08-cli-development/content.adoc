...
<4> オプション以外の引数は、配列になって `OptionParser#unknown_args` のブロック引数となります。
...
`desc` や `string` などの定義が１行で書けます。
コマンドの実行箇所は、すぐ下の `run` ブロックになります。
`version` のデフォルト実装はないので、オプションで定義しています。
