...
マクロで生成するコードは、それ単体で Crystal のコードとして完結していなければなりません。言い替えれば、 生成されたコードを別のファイルに書き出して正しくパースされるようなコードでなければなりません。
...
こうすることで、マクロが生成するコードが正しい Crystal のコードとなるため、コンパイルが通るようになります。
...
また、ループと fresh variables をは `Hash` のような構文もあります。`%変数名{キー}` と書きます。次の例を見てください。
