...
Macro とはなんでしょうか。ざっと説明すると次のようなものです。

* 「 Crystal のコードを書く」コード
* コンパイルフェーズで実行され、 Crystal のコードに展開される
* 全マクロが展開されたあとの Crystal コードが実際にコンパイルされる

これだけではイメージが湧きづらいので、Macro がどのように書かれるかを実際に見てみましょう。
次のコードを見てください。
...
. `macro` ディレクティブを用いて Macro の定義をします
. 定義した Macro を呼び出します
.. 引数が Macro に渡されます
.. 渡された引数をもとに、呼び出し箇所に Macro が展開されます
. Macro の展開後に定義されたメソッドを呼び出すことが可能です

上記のコードでの Macro 展開後は次のようになります。
...
この展開が終わった後で実際のコンパイルが行われます。

Macro のイメージは湧きましたでしょうか。
...
しかし、今後インスタンス変数が増えたとしても、Macroの呼び出し引数にその名前を渡すだけでよくなります。
重複を排除できました。

実は、今回のような `getter` の Macro は、標準ですでに搭載されています。
ですので、上記のコードは次のように書くことができます。
...
だいぶすっきりしました。このように、 Macro の恩恵を受けることですっきりとしたコードを書くことが可能です。

=== Macro の可読性

重複の除去でコードはすっきりしました。
とはいえ、すっきりしたのは Macro の呼び出し部分だけです。
自作の Macro 定義のコードはどうしても複雑になってしまいます。
Macro の理解に加え、その Macro の展開後の Crystal コードも理解しければならないからです。

Crystal のバージョン `0.20.4` 以前は、 Macro の展開後のコードを知るすべはありませんでした。
唯一のヒントは、エラーメッセージだけでした。
しかし、 Crystal のバージョン `0.20.5` から `crystal tool expand` が追加されました。
...
`--cursor` オプションでカーソル位置を指定すると、そのカーソル上の Macro の展開結果を表示することができます。
...
Macro が展開されました。
このコマンドはエディタから叩けるようにすると便利です。
設定方法は、エディタそれぞれの方法を参照してください。

この `crystal tool expand` のおかげで、 Macro のデバッグのしやすさが格段に向上しました。
...
同値性比較を行う `==` メソッドを定義しました。
このコードを、今回紹介する `def_equals` Macro を使って書くと次のようになります。
...
引数に与えた処理を並行処理します。
それぞれの処理の返り値も受け取ることができます。
次のコードは、`job1`, `job2`, `job3` を並行に実行し、返り値を受け取るコードです。

[source,crystal,indent=0]
----
      io = IO::Memory.new
include::./projects/macro/src/parallel.cr[tags=io_memory;code]
----
...
いかがでしたでしょうか。
３つの Macro を紹介しましたが、この他にも標準の Macro は存在します。
興味のある方はぜひ探してみてください。
今までのコードがずっとすっきりするので、試してみたくなることでしょう。
...
上記の `(1)` の部分では、 `macro` ディレクティブを用いて Macro の定義を書いています。
`(2)` の部分では、定義された Macro の呼び出しを行っています。
...
AST とは「Abstract Syntax Tree」の頭文字で、「抽象構文木」を意味します。
抽象構文木を構成する木構造の各要素を AST node と言います。
イメージ図を描くと次のようになります。

TODO: コードと抽象構文木の図

このように、 Crystal のコードは各 AST node で構成されています。

ここで Macro に話を戻します。
Macro は Crystal のコードを組み立てるものでした。
これは「 Macro は AST node を操作して Crystal コードを組み立てるもの」ということになります。
...
[source,crystal,indent=0]
----
include::./examples/syntax/ast_node.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/ast_node_line_number.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/ast_node_stringify.cr[tags=code]
----

`#id` は、 AST node の `MacroId` を返します。 [TODO]

[source,crystal,indent=0]
----
include::./examples/syntax/ast_node_id.cr[tags=code]
----

このように、 `Crystal::Macros::ASTNode` class には AST node を操作するためのメソッドが定義されています。
そして、それらを継承している具体的な class （ `Crystal::Macros::ArrayLiteral` など）は、それぞれの便利なメソッドが定義されています。
例えば、 `Crystal::Macros::ArrayLiteral` には `Array` classに似たメソッドが定義されています。

[source,crystal,indent=0]
----
include::./examples/syntax/ast_node_array_literal.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/macro_scope_global.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/macro_scope_global_private.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/macro_scope_class.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/macro_scope_module.cr[tags=code]
----

global スコープでは通常のメソッド定義と同じ、 class や module に関してはクラスメソッドの定義と同じです。「 Macro 定義が呼び出せない」という問題に陥った場合は、こちらの例を思い出してください。
...
[source,crystal,indent=0]
----
include::./examples/syntax/if.cr[tags=code]
----

`if` での true/false の扱いは次のようになっています。

* `false` として扱われるもの
** `Nop`, `NilLiteral`, `BoolLiteral` の false
* `true` として扱われるもの
** 上記以外
...
[source,crystal,indent=0]
----
include::./examples/syntax/for.cr[tags=code]
----

`ArrayLiteral` を渡すとfor文が回り、メソッドを定義します。
この `for` は、 `HashLiteral` にも対応しています。

[source,crystal,indent=0]
----
include::./examples/syntax/for_hash.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/for_outside.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/variadic_arguments.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/splat.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/constants.cr[tags=code]
----

一見、 Macro 以外の部分を Macro が参照しているので違和感があります。

Crystal は定数の再代入は認めていません。再代入があった場合は、 `already initialized constant XXX` というエラーと共に、コンパイル時に落とされます。

つまり、コードに書いた以上それは不変なので、 Macro でも扱えるというわけです。[TODO:いい感じに説明]

==== Nested macros

ネストした Macro も書くことができます。
つまり、「 Macro 定義を生成する Macro 」です。
内側の Macro は、外側の Macro で展開されないように `\` でエスケープする必要があります。
Crystal のマニュアルの例がわかりやすいので引用します。
次のコードを見てください。

[source,crystal,indent=0]
----
include::./examples/syntax/nested_macros.cr[tags=code]
----
...
Nested macros は、 Macro を記述で重複が多い場合に利点があります。
...
[source,crystal,indent=0]
----
include::./examples/syntax/pitfalls.cr[tags=code]
----

Macro が展開された場合、一見 Crystal の syntax を満たすコードのように見えます。
しかし、 Macro で生成されるコードは `when` ディレクティブのみなので、それだけでは Crystal の syntax を満たしません。
この場合は、 `{% begin %} ... {% end %}` を使用して、一つのコードの塊を括ってしまいます。

[source,crystal,indent=0]
----
include::./examples/syntax/pitfalls_begin_end.cr[tags=code]
----

こうすることで、 Macro が生成するコードは Crystal の syntax を満たすので、コンパイルが通ります。
陥りやすい落とし穴なので気をつけてください。

==== 型の情報にアクセスできる `@type`

Macro には特別なインスタンス変数 `@type` が用意されています。
これを使うと、コンパイル時の型の情報にアクセスできます。
実際どんなメソッドが存在しているかを見ていただいたほうがわかりやすいので、いくつかご紹介します。
`@type` は `Crystal::Macros::TypeNode` クラスです。

===== TypeNode#instance_vars

型に定義されているインスタンス変数を返します。
返り値は `Crystal::Macros::MetaVar` クラスの配列 `ArrayLiteral(MetaVar)` です。
`MetaVar` クラスは、変数やインスタンス変数を表す型で、名前（ `MetaVar#name` ）と型情報（ `MetaVar#type` ）を持っています。

[source,crystal,indent=0]
----
include::./examples/syntax/type_instance_vars.cr[tags=code]
----

===== TypeNode#methods

型に定義されているメソッドの情報を返します。
返り値は `Crystal::Macros::Def` クラスの配列 `ArrayLiteral(Def)` です。
`Def` クラスは、 `def` ディレクティブを表す型で、メソッド定義に関するさまざまな情報を持っています。
例えば、 `Def#args` は引数の情報、 `Def#return_type` はメソッドの返り値の型を表します。

[source,crystal,indent=0]
----
include::./examples/syntax/type_methods.cr[tags=code]
----

　

これらの他にもメソッドはたくさんあります。
私の調べた限りでは、組み合わせればやりたいことはできるという、必要最低限なメソッドは揃っていました。
興味のある方はぜひ調べてみてください。
...
[source,crystal,indent=0]
----
include::./examples/syntax/hooks_inherited.cr[tags=code]
----

継承した場合のみ実行されるので、 `SuperClass` には `#type_name` が存在していないことがわかります。

もう一つ、 `method_missing` の例も見てみましょう。

[source,crystal,indent=0]
----
include::./examples/syntax/hooks_method_missing.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/fresh_variables_example1.cr[tags=code]
----

これは、意図的にローカル変数を上書きして重複を排除する際には有効です。
...
[source,crystal,indent=0]
----
include::./examples/syntax/fresh_variables_example2.cr[tags=code]
----
...
[source,crystal,indent=0]
----
include::./examples/syntax/fresh_variables_example3.cr[tags=code]
----
...
また、DSLの提供も比較的簡単にできるのではないでしょうか。
Macro に慣れてくると、 Crystal 本来のコードを書くよりも Macro を書いている比率が多くなっていく気がします。
